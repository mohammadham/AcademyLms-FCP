      ...
      <div class="flex-shrink-0 mt-3">
        <ul class="list-unstyled ps-0">
          ...
          <?php if(addon_status('FCP')): ?>
            <li class="bg-light">
              <button class="btn btn-toggle d-inline-flex align-items-center rounded border-0 text-dark text-16px fw-500 collapsed" data-bs-toggle="collapse" data-bs-target="#FCP-category-collapse" aria-expanded="false">
                <i class="fas fa-file me-2"></i>
                <?php echo get_phrase('Coupons'); ?>
              </button>
              <div class="collapse" id="FCP-category-collapse">
                <ul class="btn-toggle-nav list-unstyled fw-normal pb-1 small bg-white pt-2">
                  <?php
                $FCP_categories = $this->db->get('FCP_category')->result_array();
                foreach ($FCP_categories as $key => $FCP_categories):?>
                  <li>
                    <a href="<?php echo site_url('FCP?category='.$FCP_categories['slug'].'&price=all&rating=all') ?>" class="btn btn-toggle-list d-inline-flex align-items-center rounded border-0 text-dark text-15px fw-400 py-2 w-100"> <?php echo $FCP_categories['title']; ?></a>
                  </li>
                  <?php endforeach; ?>
                  <li>
                    <a href="<?php echo site_url('FCP') ?>" class="btn btn-toggle-list d-inline-flex align-items-center rounded border-0 text-dark text-15px fw-400 py-2 w-100"> <i class="fas fa-list me-2"></i> <?php echo get_phrase('All FCPs'); ?></a>
                  </li>
                </ul>
              </div>
            </li>
